...
class Leave {
	...
	cy.wait(2000)
		...
		if (ManagerLevel == 1) {
			...
			cy.get('tbody > :nth-child(2) > :nth-child(2)').invoke('text').then((CODE_2) => {
			
				softExpect(CODE_2.trim()).to.eq(managerID_2)
			})

			cy.get('tbody > :nth-child(2) > :nth-child(3)').invoke('text').then((NAME_2) => {
				softExpect(NAME_2.trim()).to.eq(ManagerFirstName_2 + ' '+ManagerLastName_2)
			})

			cy.get('tbody > :nth-child(2) > :nth-child(4)').invoke('text').then((STATUS_2) => {
				softExpect(STATUS_2.trim()).to.eq(ManagerStatus_2)
			})

			cy.get('tbody > :nth-child(2) > :nth-child(5)').invoke('text').then((MANAGERREMARKS_2) => {
				softExpect(MANAGERREMARKS_2.trim()).to.eq(ManagerRemark_2.trim())
			})
		}
		...
		cy.get('.form-horizontal > :nth-child(1) > :nth-child(4) > :nth-child(2)').invoke('text').then((TodayDate) => {
			...
			softExpect(TodayDate.trim()).to.eq(todayDate)
		})
		...
		cy.get('.mt-4 > tbody > :nth-child(1) > :nth-child(4)').invoke('text').then((text) => {
			softExpect(text.trim()).to.eq(ManagerStatus_1.trim())
			//expect(text.trim()).to.contains(ManagerStatus_1)
		})
		cy.get('.mt-4 > tbody > :nth-child(1) > :nth-child(5)').invoke('text').then((text) => {
			softExpect(text.trim()).to.eq(ManagerRemark_1.trim())
	...
	verifyNotificationAtManager(employeeID, EmployeeFirstName, EmployeeLastName, managerID,
		ManagerFirstName, ManagerLastName, department, designation, employeeJoiningDate, todayDate,
		leaveType, leaveTypeValue, balance, leaveFromDate, leaveToDate, leaveFromDayType,
		leaveToDayType, Reason, LeaveStation, VacationAddress, ContactNumber, ReliverCode, ReliverName,
		APPROVERS, STATUS, ManagerRemark_1, ReliverSetting, leaveDay, ManagerLevel, managerID_2, ManagerFirstName_2,
	...
	approveLeave(employeeID, EmployeeFirstName, EmployeeLastName, managerID,
		ManagerFirstName, ManagerLastName, department, designation, employeeJoiningDate, todayDate,
		leaveType, leaveTypeValue, balance, leaveFromDate, leaveToDate, leaveFromDayType,
		leaveToDayType, Reason, LeaveStation, VacationAddress, ContactNumber, ReliverCode, ReliverName,
		APPROVERS, STATUS, ManagerRemark_1, ReliverSetting, leaveDay, ManagerLevel, managerID_2, ManagerFirstName_2,
					...
					cy.get('#tblData > tbody > tr:nth-child(' + num + ')>td:nth-child(8)').invoke('text').then((LeaveDate) => {
						...
						cy.get('#tblData > tbody').find('tr').then(listing => {
							...
							if (EmpCode.trim() == employeeID && LeaveType.trim() == leaveType || LeaveType.trim() == leaveTypeValue && LeaveDate.trim() == leaveFromDate) {
								...
								cy.get('.tdReason>input').type(ManagerRemark_2)
	...
	verifyLeaveInTeamReport(employeeID, EmployeeFirstName, EmployeeLastName, managerID,
		ManagerFirstName, ManagerLastName, department, designation, employeeJoiningDate, todayDate,
		leaveType, leaveTypeValue, balance, leaveFromDate, leaveToDate, leaveFromDayType,
		leaveToDayType, Reason, LeaveStation, VacationAddress, ContactNumber, ReliverCode, ReliverName,
		APPROVERS, STATUS, ManagerRemark_1, ReliverSetting, leaveDay, ManagerLevel, managerID_2, ManagerFirstName_2,
					...
					cy.xpath("//div[@id='divleavereportmanager']//table/tbody/tr[" + num + "]/td[4]").invoke('text').then((Fromdate) => {
						...
						if (EmpCode.trim() == employeeID && Fromdate.trim() == leaveFromDate) {
							cy.xpath("//div[@id='divleavereportmanager']//table/tbody/tr[" + num + "]/td[9]").invoke('text').then((text) => {
								softExpect(text.trim()).to.eq(STATUS);
		...
		cy.get('[data-title="Leave Type"] > .btn').click({ force: true })
		this.verifyLeaveDetails(employeeID, EmployeeFirstName, EmployeeLastName, managerID,
			ManagerFirstName, ManagerLastName, department, designation, employeeJoiningDate, todayDate,
			leaveType, leaveTypeValue, balance, leaveFromDate, leaveToDate, leaveFromDayType,
			leaveToDayType, Reason, LeaveStation, VacationAddress, ContactNumber, ReliverCode, ReliverName,
	...
	verifyNotificationAtEmployee(employeeID, EmployeeFirstName, EmployeeLastName, managerID,
		ManagerFirstName, ManagerLastName, department, designation, employeeJoiningDate, todayDate,
		leaveType, leaveTypeValue, balance, leaveFromDate, leaveToDate, leaveFromDayType,
		leaveToDayType, Reason, LeaveStation, VacationAddress, ContactNumber, ReliverCode, ReliverName,
		APPROVERS, STATUS, ManagerRemark_1, ReliverSetting, leaveDay, ManagerLevel, managerID_2, ManagerFirstName_2,
	...
	verifyStatusAtEmployee(employeeID, EmployeeFirstName, EmployeeLastName, managerID,
		ManagerFirstName, ManagerLastName, department, designation, employeeJoiningDate, todayDate,
		leaveType, leaveTypeValue, balance, leaveFromDate, leaveToDate, leaveFromDayType,
		leaveToDayType, Reason, LeaveStation, VacationAddress, ContactNumber, ReliverCode, ReliverName,
		APPROVERS, STATUS, ManagerRemark_1, ReliverSetting, leaveDay, ManagerLevel, managerID_2, ManagerFirstName_2,
	...
	verifyLeaveInMyReport(employeeID, EmployeeFirstName, EmployeeLastName, managerID,
		ManagerFirstName, ManagerLastName, department, designation, employeeJoiningDate, todayDate,
		leaveType, leaveTypeValue, balance, leaveFromDate, leaveToDate, leaveFromDayType,
		leaveToDayType, Reason, LeaveStation, VacationAddress, ContactNumber, ReliverCode, ReliverName,
		APPROVERS, STATUS, ManagerRemark_1, ReliverSetting, leaveDay, ManagerLevel, managerID_2, ManagerFirstName_2,
	...
	verifyLeaveInEmployeeReport(employeeID, EmployeeFirstName, EmployeeLastName, managerID,
		ManagerFirstName, ManagerLastName, department, designation, employeeJoiningDate, todayDate,
		leaveType, leaveTypeValue, balance, leaveFromDate, leaveToDate, leaveFromDayType,
		leaveToDayType, Reason, LeaveStation, VacationAddress, ContactNumber, ReliverCode, ReliverName,
		APPROVERS, STATUS, ManagerRemark_1, ReliverSetting, leaveDay, ManagerLevel, managerID_2, ManagerFirstName_2,
