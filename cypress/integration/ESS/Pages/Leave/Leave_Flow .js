...
class OnDuty {
     ...
     verifyNoti() {
		...
		cy.origin(Cypress.env('url'), (username, userPass) => {
        ...
        cy.xpath("//form[@id='UnapprovedLeaveForm']//tbody/tr[1]/td[3]").invoke('text').then((text) => {
            ...
            cy.xpath("//form[@id='UnapprovedLeaveForm']//tbody/tr[1]/td[6]").invoke('text').then((text) => {
			  ...
			  cy.xpath("//button[contains(@onclick,'displayModal')]").click({force: true})
			   ...
			   cy.get('.form-horizontal > :nth-child(1) > :nth-child(1) > :nth-child(2)').invoke('text').then((text) => {	
				expect(text.trim()).to.contains(EmployeeFirstName +" "+ EmployeeLastName)
				}) 

				cy.get('.form-horizontal > :nth-child(2) > :nth-child(1) > :nth-child(2)').invoke('text').then((text) => {
					expect(text.trim()).to.contains(ManagerFirstName + ManagerLastName)
					}) 
				
				cy.get('.form-horizontal > :nth-child(1) > :nth-child(2) > :nth-child(2)').invoke('text').then((text) => {	
					expect(text.trim()).to.contains(employeeID)
					}) 
								
			 cy.get('.form-horizontal > :nth-child(2) > :nth-child(2) > :nth-child(2)') .invoke('text').then((text) => {
				expect(text.trim()).to.contains(employeeJoiningDate.trim())
				}) 

			 cy.get('.form-horizontal > :nth-child(1) > :nth-child(3) > :nth-child(2)') .invoke('text').then((text) => {
				expect(text.trim()).to.contains(todayDate)
				}) 
				
				cy.get('.form-horizontal > :nth-child(2) > :nth-child(3) > :nth-child(2)') .invoke('text').then((text) => {
					expect(text.trim()).to.contains(leaveTypeValue)
					}) 

			 cy.get('.form-horizontal > :nth-child(3) > :nth-child(1) > :nth-child(2)') .invoke('text').then((text) => {
				cy.log(text.trim())
				var data = text.trim()
			 ...
			 cy.get('.form-horizontal > :nth-child(3) > :nth-child(2) > :nth-child(2)').invoke('text').then((text) => {
    ...
    verifyRequstedLeaveDetailsAtManager(employeeID,EmployeeFirstName,EmployeeLastName, managerID, ManagerFirstName, ManagerLastName, 
		leaveType, leaveTypeValue, leaveFromDate, leaveToDate, todayDate, employeeJoiningDate,leaveFromDayType, leaveToDayType, 
		Reason, LeaveStation ,VacationAddress, ContactNumber, ReliverCode, ReliverName,
		APPROVERS, STATUS, MANAGERREMARKS) {
		cy.task('readXlsx', { file: 'cypress/fixtures/Leave/LeaveFlow.xlsx', sheet: 'LeaveFlow' }).then((rows) => {
			var rowsLength = rows.length;
			cy.writeFile("cypress/fixtures/Leave/LeaveFlow"+".json", {rows})
		  })
		...
		// cy.fixture("Leave/LeaveFlow"+".json",).then((excelData) => {
		...
		// 				expect(EmpCode.trim()).to.equal(data.EmployeeCode)
		// 			})  
		// 		}) 
		// 		})
		// 	})
    ...
    verifyLeaveInTeamReport(employeeID, leaveFromDate) {
        const { softAssert, softExpect, softTrue } = chai;
		cy.wait(10000)
		cy.visit(Cypress.env('essUrl') +'Leave/TeamReport/MyTeamLeaveReport?Menu=leavedetails');
		cy.wait(500)
			cy.get('#DataTables_Table_0_wrapper>  table >tbody > tr').each(function(row, i){	
			var num = parseFloat(i+1)	
			var len = 
			
			cy.get('#DataTables_Table_0_wrapper>  table >tbody > tr:nth-child('+num+') >:nth-child(1)').invoke('text').then((EmpCode) => {
			cy.get('#DataTables_Table_0_wrapper>  table >tbody > tr:nth-child('+num+') >:nth-child(4)').invoke('text').then((Fromdate) => {
		
				if (EmpCode.trim() == employeeID && Fromdate.trim() == leaveFromDate)
				{
					cy.get('#DataTables_Table_0_wrapper>  table >tbody > tr:nth-child('+num+') >:nth-child(9)').invoke('text').then((text) => {
