describe('Proof Details Import', function() {
	
	var proofdetailsfilePath= 'ProofDetailsImport.xlsx';
	...
	it('Save Setting for Proof Details Import',function() {
		cy.server()	
		cy.wait(1000)
		...
		cy.wait(2000)
		cy.get('#excelImport').select('Proof Details Import',{force: true})
		...
		cy.wait(2000)
		 cy.get(".toast-message").invoke('text').then((text) => {
			...
			cy.get(".toast-message").eq(0).click({force: true})
			  ...
			  cy.get('#SettingNameNew').type('ProofDetailsImport')
			}
		 })
		...
		cy.wait(1000)
		 cy.get(".toast-message").invoke('text').then((text) => {
			...
			cy.get(".toast-message").eq(0).click({force: true})
			 ...
			 cy.get('#SUBFLD30').select('C',{force: true})
			}
		 })
	...
	it('Upload Proof Details Import excel file',function() {
		cy.visit(Cypress.env('url')+'ITax/Utilities/IncomeTaxImport?import')
		cy.wait(1000)
		cy.get('#excelImport').select('Proof Details Import',{force: true})
		cy.wait(2000)
		cy.get('#SettingName').select('ProofDetailsImport',{force: true})
		
		cy.wait(1000)
		cy.fixture(proofdetailsfilePath, 'binary')
		.then(Cypress.Blob.binaryStringToBlob)
		.then(fileContent => {
		cy.get('#file').upload({
		fileContent,
		fileName: proofdetailsfilePath,
		...
		cy.wait(2000)
		cy.get('#ExcelSheetName').select('ProofDetails',{force: true})
		cy.wait(1000)
