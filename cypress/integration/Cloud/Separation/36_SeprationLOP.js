describe('Sepration with LOP', function() {
...
/*	it('Save Salary Details of EMp',function(){
	...
	it('Delete monthly input && Save monthly input',function(){
		...
		cy.xpath("//tbody/tr[2]/td[1]/input[1]").click({force: true})
		cy.xpath("//tbody/tr[2]/td[1]/input[1]").clear()
		cy.xpath("//tbody/tr[2]/td[1]/input[1]").type('3')
		cy.wait(2000)
	...
	it('Payroll process on pervious month',function(){
		...
		cy.get('#globalSearch').type(employeeId)
		cy.wait(4000)
		cy.contains('li', 'SeprationLOP test(CY16)').click({force: true})
		cy.wait(3000)
		cy.get('#payroll_detail_tab').click({force: true})
		cy.wait(1000)
		cy.get('#Utilities_PayrollProcess').click({force: true})
...
*/
	it('Employee Separation for LOP',function(){
		cy.wait(2000)
		cy.server()
	...
	it('Full & Final Settlement with LOP',function(){
		...
		cy.get('#resignationDate').then($input => {
			expect($input.val()).to.contain('08/01/'+Cypress.env('FinancialYear_From'))
		})
		
		cy.get('#relievingDate').then($input => {
			expect($input.val()).to.contain('27/03/'+Cypress.env('FinancialYear_To'))
		})
		...
		cy.get('#noticeperiod').type('45')
		
		
		cy.get('#reportname').click({force: true})
		cy.get('#reportname').clear()
		cy.get('#reportname').type('test report with LOP')
		
		cy.get('#notes').click({force: true})
		cy.get('#notes').clear()
		cy.get('#notes').type('test fillnfinal with LOP')
